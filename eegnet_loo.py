...
for j in range(len(persons)):
    ...
    test_ids = [j]
    # Training data
    X_train = np.concatenate([new_resampled[i].get_data() for i in train_ids], axis=0)
    y_train = np.concatenate([list_labels[i] for i in train_ids], axis=0)
    ...
    for bs in batch_sizes:
        for lr in lrs:
            ...
            if f'lr={lr:.4f}, bs={bs}' in scores:
                ...
                scores[f'lr={lr:.4f}, bs={bs}'] = {persons[j]: score}
        with open('results_loo/eegnet.json', 'w') as f:
            json.dump(scores, f)
